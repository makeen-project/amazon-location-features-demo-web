...
import { FC, MutableRefObject, ReactNode, lazy, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
import { EsriMapEnum, ResponsiveUIEnum, TriggeredByEnum } from "@demo/types/Enums";
...
const {
	ROUTES: { DEMO },
	MAP_RESOURCES: { MAX_BOUNDS, MAP_ITEMS },
...
interface AmplifyConfigInitProps {
	children: ReactNode;
}

const AmplifyConfigInit: FC<AmplifyConfigInitProps> = ({ children }) => {
	const { identityPoolId, region, userPoolId, userPoolClientId, userDomain, configureAmplify } = useAmplifyAuth();

	useEffect(() => {
		console.log("AmplifyConfigInit MOUNTED");

		return () => {
			console.log("AmplifyConfigInit UNMOUNTED");
		};
	});
		...
		<AmplifyConfigInit>
			{!!credentials?.identityId ? (
				...
				>
									...
									{show.sidebar && (
										<Sidebar
											...
											onShowAuthTrackerDisclaimerModal={() =>
												setShow(s => ({ ...s, authTrackerDisclaimerModal: true }))
											}
							...
							style={
								isDesktop
									? {
											display: "flex",
											fontSize: "0.77rem",
											color: currentMapStyle.toLowerCase().includes("dark")
												? "var(--white-color)"
												: "var(--black-color)",
					...
					/>
					<FeedbackModal
						open={show.openFeedbackModal}
						onClose={() => setShow(s => ({ ...s, openFeedbackModal: false }))}
					/>
					<SettingsModal
						open={show.settings}
						onClose={() => {
